...
def run(mol):
    ...
    pymol = gto.M(
        ...
        unit       = params.mol_param['units'])
    
    # if var.d3_inp['xc']='hf', use canonical hf orbitals
    if params.scf_param['xc'] == 'hf': 
        if params.mol_param['use_df']:
            if mol.spin == 0.:
    ...
    # this is a DFT computation
        ...
        mf.xc = params.scf_param['xc']
        
    # save integrals
    if params.mol_param['use_df']:
        mf.with_df.auxbasis       = params.mol_param['ri_basis']
        # this will be generated during the calculation, 
        # saved upon completion
        mf.with_df._cderi_to_save = params.file_param['2ei']
    else:
        if params.scf_param['xc'] != 'hf':
            mf.with_df = False    
    ...
    if params.mol_param['use_df']:
        ...
        df.outcore.general(pymol, ij_trans, params.file_param['2ei'], 
                     params.mol_param['ri_basis'], dataname='eri_mo')        
    else:
        eri_ao = pymol.intor('int2e_sph', aosym='s8')
        eri_mo = ao2mo.incore.full(eri_ao, mf.mo_coeff)        
        with h5py.File(params.file_param['2ei'], 'w') as f:
            f['eri_mo'] = eri_mo
    ...
    with h5py.File(params.file_param['1ei'], 'w') as f:
        f['hcore_mo'] = h1_mo
