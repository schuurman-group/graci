...
@timing.timed
def refine_ref_space_pruned(ci_method):
    """
    Refinement of the reference space for a pruned
    calculation
    """
    ...
    ci_dspunits = np.array(mrci_wfn.dsp_units, dtype=int)
    ...
    # Refine the reference space
    args = (ci_confunits, ref_confunits, ci_ciunits, ci_dspunits,
            nstates, nextra, cthrsh, min_norm, ref_nconf)
    (confunits_ref, min_norm, ref_nconf) = \
            libs.lib_func('refine_ref_space_pruned', args)
