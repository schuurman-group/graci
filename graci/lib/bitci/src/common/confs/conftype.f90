...
module conftype
  ...
  type, public :: mrcfg
     ...
     ! Number of CSFs as a function of the number of open shells
     integer(is), allocatable :: ncsfs(:)
...
!----------------------------------------------------------------------
    ...
    do iconf=1,cfg%n0h
       ...
       cfg%csfdim=cfg%csfdim+cfg%ncsfs(nopen)

       ! Offset
       cfg%csfs0h(iconf)=sum
       sum=sum+cfg%ncsfs(nopen)
...
!######################################################################
  subroutine finalise(cfg)
    ...
    !
    if (allocated(cfg%ncsfs))    deallocate(cfg%ncsfs)
