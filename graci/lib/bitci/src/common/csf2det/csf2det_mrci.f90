...
!            determinant basis
!######################################################################
  subroutine det_trans(cfg,nroots,csfdim,detdim,vec_csf,vec_det,det)
...
!----------------------------------------------------------------------
    call bitstrings_detbas(cfg,detdim,det,dphase)
...
!######################################################################
  subroutine bitstrings_detbas(cfg,detdim,det,dphase)
...
!----------------------------------------------------------------------
    ...
    do iconf=1,cfg%n0h
       ...
       call sop_socc_list(cfg%sop0h(:,:,iconf),n_int_I,socc,nmo,nopen)
...
!----------------------------------------------------------------------
    ...
    do iconf=1,cfg%n1I
       ...
       do idet=1,ndet
          ...
          ! Get the doubly-occupied phase factors
          dphase(counter)=docc_phase(n_int,det(:,:,counter),&
               cfg%sop1I(:,:,iconf))

          ! Fill in the doubly-occupied MOs
          do k=1,2
             det(:,k,counter)=ior(det(:,k,counter), &
                  cfg%sop1I(:,2,iconf))
          enddo
