...
!----------------------------------------------------------------------
    ...
    case(1)
       call package_integrals_nexci1(&
            ...
            bpattern,kpattern,Vpqrs,m2c,socc,nsocc,nbefore,&
            Dw,ndiff,icase)
    case(2)
       ...
       do bcsf=bcsfs(bconf),bcsfs(bconf+1)-1
          ...
          case(1) ! Bra and ket configurations linked by a single
             ...
             harr(bomega,komega)=hij_single_mrci(&
                  bomega,komega,bnopen,knopen,pairindx(1),&
                  icase,bpattern,kpattern,Vpqrs,socc,nsocc,&
                  ndiff,hlist(1),plist(1))
          case(2) ! Bra and ket configurations linked by two
                  ! excitations
             harr(bomega,komega)=hij_double_mrci(&
                  bomega,komega,bnopen,knopen,pairindx(1:2),&
...
!----------------------------------------------------------------------
    if (ldftmrci) call hij_dftmrci_batch(harr(1:bnsp,1:knsp),&
         bnsp,knsp,bavii,kavii)
