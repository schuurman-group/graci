...
!######################################################################
#ifdef CBINDING
subroutine prune_ref_space(nroots,nextra,confscr,nconf,vecscr) &
...
!----------------------------------------------------------------------
  ...
  do irrep=0,nirrep-1
     ...
     ! this irrep
     call get_deadwood_new(nconf(irrep),refdim(irrep),nroots(irrep),&
          nextra(irrep),vec0,offset,idead(istart:iend))
...
!######################################################################
! get_deadwood_new: determines the indices of the deadwood
!                   configurations under the condition that at least
!                   nroots CSFs remain in the reference space
!######################################################################
subroutine get_deadwood_new(nconf,refdim,nroots,nextra,vec0,offset,&
     idead)
  ...
  ! Dimensions
  integer(is), intent(in)  :: nconf,refdim,nroots,nextra
  ...
  ! Everything else
  integer(is)              :: n,iconf,icsf,icsf1,nsurvive,i1
  integer(is), allocatable :: indx(:,:),confmap(:),ilast(:)
...
!----------------------------------------------------------------------
  ...
  allocate(indx(refdim,nroots))
  indx=0

  allocate(confmap(refdim))
  confmap=0

  allocate(ilast(nroots))
  ilast=0
...
!----------------------------------------------------------------------
  ...
  do n=1,nroots
     ...
     ! Sort the CSFs for this state by absolute coefficient value
     call dsortindxa1('D',refdim,abs(vec0(:,n)),indx(:,n))
...
!----------------------------------------------------------------------
  ...
  do n=1,nroots
     ...
     ! Loop over CSFs (importance ordered)
     do icsf1=1,refdim
        icsf=indx(icsf1,n)
        ...
        if (abs(vec0(icsf,n)) > cthrsh) then
           ! Above threshold coefficient: flag as not dead
           idead(iconf)=0
        else
           ! Below threshold coefficient: may still be needed
           ! if too few CSFs are selected
           if (ilast(n) == 0) ilast=icsf1-1
        endif
...
!----------------------------------------------------------------------
  ...
  do iconf=1,nconf
     ...
     ! Accumulate the no. CSFs generated by this configuration
     ! if it isn't dead
     if (idead(iconf) == 0) nsurvive=nsurvive+&
          offset(iconf+1)-offset(iconf)

  enddo

!----------------------------------------------------------------------
! Have we retained enough CSFs? If not then add some extra confs back
! in
!----------------------------------------------------------------------
  do while(nsurvive < nroots+nextra)

     ! Loop over roots
     do n=1,nroots

        ! CSF and configuration indices
        icsf=indx(ilast(n),n)
        iconf=confmap(icsf)

        ! Is this conf flagged as dead?
        if (idead(iconf) == 1) then

           ! Flag the conf as not dead
           idead(iconf)=0

           ! Update the no. surviving CSFs
           nsurvive=nsurvive+offset(iconf+1)-offset(iconf)
           
        endif
        
        ! Update the last CSF counter
        ilast(n)=ilast(n)+1
...
end subroutine get_deadwood_new
