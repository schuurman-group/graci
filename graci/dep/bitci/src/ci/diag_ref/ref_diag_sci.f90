...
!######################################################################
  ...
  subroutine partition_confs_new(nroots,nconf,offset,csfdim,hii,nP,iP,&
       nQ,iQ,offsetP,offsetQ,confmap,&
       averageii,hiiP,conf,sop,n_int_I,m2c,irrep)
    ...
    ! On-diagonal Hamiltonian matrix elements for the P space
    real(dp), intent(out)    :: hiiP(csfdim)
    ...
    ! Everything else
    integer(is)              :: iconf,icsf,i
    integer(is)              :: count,countP,countQ
    integer(is)              :: totalP,totalQ
    integer(is)              :: nlow
    integer(is), allocatable :: isurvive(:)
    integer(is)              :: hscr,nrec
    integer(is)              :: ncsf,csfdimP
    integer(is)              :: vecscr


    integer(is)              :: icsfP,iconfP
    integer(is), allocatable :: indxP(:)
    integer(is), allocatable :: confmapP(:)
    real(dp), allocatable    :: vecP(:,:),EP(:),absvec(:)
    real(dp)                 :: targ,sumsq
...
!----------------------------------------------------------------------
! Determine the P space configurations from which each P space CSF is
! generated
!----------------------------------------------------------------------
    allocate(confmapP(csfdimP))
    confmapP=0
    ...
    do iconf=1,nP
       ...
       do icsf=offsetP(iconf),offsetP(iconf+1)-1

          ! Fill in the P space CSF-to-conf mapping array
          confmapP(icsf)=iconf
...
! Diagonalise the P space Hamiltonian
!----------------------------------------------------------------------
    call diag_full(hscr,nrec,csfdimP,hiiP,irrep,nroots,vecscr,'refvec')
...
!----------------------------------------------------------------------
    allocate(absvec(csfdimP))
    absvec=0.0d0

    allocate(indxP(csfdimP))
    indxP=0
    ...
    do i=1,nroots
       ...
       ! Sort the eigenvector by absolute value
       absvec=abs(vecP(:,i))
       call dsortindxa1('D',csfdimP,absvec,indxP)
       ...
       do icsfP=1,csfdimP
          ...
          ! Which P space configuration generates this
          ! P space CSF? We will need a P space CSF-to-conf map
          ! for this...
          iconfP=confmapP(indxP(icsfP))

          ! Which full space configuration does this P space
          ! configuration map onto?
          iconf=iP(iconfP)
          ...
          ! Exit if we have reached our target squared notm
          if (sumsq >= targ) then
             exit
          endif
