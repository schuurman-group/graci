...
module rdm
  ...
  ! Spin-coupling coefficients (one column per OpenMP thread)
...
!######################################################################
  subroutine rdm_mrci(cfg,csfdim,nroots,vec,rho)
    ...
    ! Per-thread density matrix accumulators
    real(dp), allocatable   :: rho_thr(:,:,:,:)
...
!----------------------------------------------------------------------
    ...
    ! Per-thread spin-coupling and density matrix accumulators
    nthreads=omp_get_max_threads()

    arrdim=maxval(ncsfs(0:nomax))**2
    allocate(scc(arrdim,nthreads))
    scc=0.0d0

    allocate(rho_thr(nmo,nmo,nroots,nthreads))
    rho_thr=0.0d0

!----------------------------------------------------------------------
! (1) On-diagonal elements
!----------------------------------------------------------------------
    call rdm_diag(cfg,csfdim,nroots,vec,rho_thr,nthreads)

!----------------------------------------------------------------------
! (2) Ref - Ref contributions to the 1-RDMs
!----------------------------------------------------------------------
    call rdm_0h_0h(cfg,csfdim,nroots,vec,rho_thr,nthreads)

!----------------------------------------------------------------------
! (3) Ref - 1H contributions to the 1-RDMs
!----------------------------------------------------------------------
    call rdm_0h_1h(cfg,csfdim,nroots,vec,rho_thr,nthreads)

!----------------------------------------------------------------------
! (4) Ref - 2H contributions to the 1-RDMs
!----------------------------------------------------------------------
    call rdm_0h_2h(cfg,csfdim,nroots,vec,rho_thr,nthreads)

!----------------------------------------------------------------------
! (5) 1H - 1H contributions to the 1-RDMs
!----------------------------------------------------------------------
    call rdm_1h_1h(cfg,csfdim,nroots,vec,rho_thr,nthreads)

!----------------------------------------------------------------------
! (6) 2H - 1H contributions to the 1-RDMs
!----------------------------------------------------------------------
    call rdm_2h_1h(cfg,csfdim,nroots,vec,rho_thr,nthreads)

!----------------------------------------------------------------------
! (6) 2H - 2H contributions to the 1-RDMs
!----------------------------------------------------------------------
    call rdm_2h_2h(cfg,csfdim,nroots,vec,rho_thr,nthreads)

!----------------------------------------------------------------------
! Reduce per-thread 1-RDMs into the output density matrix
!----------------------------------------------------------------------
    do tid=1,nthreads
       rho=rho+rho_thr(:,:,:,tid)
    enddo
...
!                1-RDMs
!######################################################################
  subroutine rdm_diag(cfg,csfdim,nroots,vec,rho_thr,nthreads)
    ...
    ! Per-thread density matrix accumulator
    real(dp), intent(inout) :: rho_thr(nmo,nmo,nroots,nthreads)
    ...
    integer(is)             :: i,n,ista,imo,ioff,tid
    integer(is)             :: n_int_I
    real(dp)                :: c2
...
!----------------------------------------------------------------------
    ...
    !$omp& private(iconf,ista,icsf,i,imo,c2,socc,docc,nsocc,ndocc,tid) &
    ...
    do iconf=1,cfg%n0h
       ...
       do ista=1,nroots
          ...
          do icsf=cfg%csfs0h(iconf),cfg%csfs0h(iconf+1)-1
             ...
             do i=1,nsocc
                imo=cfg%m2c(socc(i))
                rho_thr(imo,imo,ista,tid)=rho_thr(imo,imo,ista,tid)+c2
             enddo

             ! Doubly-occupied MO contributions to the 1-RDM
             do i=1,ndocc
                imo=cfg%m2c(docc(i))
                rho_thr(imo,imo,ista,tid)=rho_thr(imo,imo,ista,tid)+2.0d0*c2
             enddo
...
!----------------------------------------------------------------------
    if (cfg%n1I > 0) then
       ...
       !$omp parallel do default(shared) &
       !$omp& private(n,ioff,ista,icsf,i,imo,c2,socc,docc,nsocc,ndocc,tid) &
       !$omp& schedule(dynamic)
       do n=1,cfg%n1h
...
!                 1-RDMs
!######################################################################
  subroutine rdm_0h_0h(cfg,csfdim,nroots,vec,rho_thr,nthreads)
...
!----------------------------------------------------------------------
    ...
    do ikconf=1,cfg%n0h-1
       ...
       !$omp parallel do default(shared) &
       !$omp& private(ibconf,nexci,bnopen,bnsp,hlist,plist,i,a,ista, &
       !$omp&         ikcsf,ibcsf,komega,bomega,counter,kcoe,bcoe, &
       !$omp&         prod,tid) &
       !$omp& schedule(dynamic)
       do ibconf=ikconf+1,cfg%n0h
          ...
          ! Get the spin-coupling coefficients (per-thread column)
          scc(1:knsp*bnsp,tid)=spincp_coeff(knsp,bnsp,ksop_full,plist(1),&
          ...
          do ista=1,nroots
             ...
             do ikcsf=cfg%csfs0h(ikconf),cfg%csfs0h(ikconf+1)-1
                ...
                do ibcsf=cfg%csfs0h(ibconf),cfg%csfs0h(ibconf+1)-1
                   ...
                   prod=kcoe*bcoe*scc(counter,tid)
                   rho_thr(i,a,ista,tid)=rho_thr(i,a,ista,tid)+prod
                   rho_thr(a,i,ista,tid)=rho_thr(a,i,ista,tid)+prod
...
!                 to the MRCI 1-RDMs
!######################################################################  
  subroutine rdm_0h_1h(cfg,csfdim,nroots,vec,rho_thr,nthreads)
          ...
          if (do1I) then
             if (nac <= 3 .and. cfg%off1I(n) /= cfg%off1I(n+1)) then
                call rdm_batch(&
                     ...
                     csfdim,nroots,vec,rho_thr,nthreads,tid,cfg%m2c,.false.)
             endif
          endif
...
!######################################################################  
  subroutine rdm_0h_2h(cfg,csfdim,nroots,vec,rho_thr,nthreads)
...
!            to the MRCI 1-RDMs
!######################################################################
  subroutine rdm_1h_1h(cfg,csfdim,nroots,vec,rho_thr,nthreads)
...
!----------------------------------------------------------------------
    ...
    !$omp parallel do default(shared) &
    !$omp& private(kn,bn,nac1,nac,ikconf,kconf_full,ksop_full, &
    !$omp&         nbefore,knopen,knsp,tid) &
    !$omp& firstprivate(kconf_int,ksop_int) &
    !$omp& schedule(dynamic)
    do kn=1,cfg%n1h
             ...
             do ikconf=cfg%off1I(kn),cfg%off1I(kn+1)-1
                ...
                if (nac <= 3 .and. bn >= kn &
                   ...
                   call rdm_batch(&
                        ...
                        csfdim,nroots,vec,rho_thr,nthreads,tid,cfg%m2c,.true.)
                endif
...
!######################################################################
  subroutine rdm_2h_1h(cfg,csfdim,nroots,vec,rho_thr,nthreads)
...
!----------------------------------------------------------------------
    ...
    !$omp parallel do default(shared) &
    !$omp& private(kn,bn,nac1,nac,ikconf,kconf_full,ksop_full, &
    !$omp&         bconf1h_full,nbefore,knopen,knsp,tid) &
    !$omp& firstprivate(kconf_int,ksop_int) &
    !$omp& schedule(dynamic)
    do kn=1,cfg%n2h
...
!######################################################################
  subroutine rdm_2h_2h(cfg,csfdim,nroots,vec,rho_thr,nthreads)
...
!----------------------------------------------------------------------
    ...
    !$omp parallel do default(shared) &
    !$omp& private(kn,bn,nac1,nac,ikconf,kconf_full,ksop_full, &
    !$omp&         bconf2h_full,nbefore,knopen,knsp,tid) &
    !$omp& firstprivate(kconf_int,ksop_int) &
    !$omp& schedule(dynamic)
    do kn=1,cfg%n2h
...
!######################################################################
  subroutine rdm_batch(bn,ikconf,kconf,ksop,knopen,knsp,knbefore,&
       ...
       csfdim,nroots,vec,rho_thr,nthreads,tid,m2c,same_class)
    ...
    ! OpenMP info
    integer(is), intent(in) :: nthreads,tid
    ...
    integer(is)             :: nexci,bnopen,bnsp,i,a,counter
    ...
    do ibconf=boffset(bn),boffset(bn+1)-1
       ...
       ! Get the spin-coupling coefficients (per-thread column of scc)
       scc(1:knsp*bnsp,tid)=spincp_coeff(knsp,bnsp,ksop,plist(1),hlist(1),&
            knopen,knbefore)
