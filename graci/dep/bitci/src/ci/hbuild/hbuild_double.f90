...
!######################################################################
! save_hij_double_selected: Saves the non-zero off-diagonal elements
...
!######################################################################
  subroutine save_hij_double_selected(nsel,isel,offset_sel,nconf,hdim,&
    ...
    ! Everything else
    integer(is)                  :: i,j,iconf,kconf,bconf
    integer(is)                  :: icsf,kcsf,bcsf
    integer(is)                  :: iomega,komega,bomega
    integer(is)                  :: nsp,count,blim1,blim2,&
                                    klim1,klim2,bnsp,knsp
...
!----------------------------------------------------------------------
    ...
    if (verbose) &
         call report_times(twall_end-twall_start,tcpu_end-tcpu_start,&
         'save_hij_double_selected')
    
    return
    
  end subroutine save_hij_double_selected
